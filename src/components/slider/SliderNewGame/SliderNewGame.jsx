...
import SwiperCore, { FreeMode, Navigation, Thumbs, EffectCube, Autoplay, Pagination } from 'swiper';
import 'swiper/css';
...
const gameItems = [
  {
    ...
    'date' : '1/1/2023',
    'price' : '120000',
    'description' : 'Elden Ring là một trò chơi nhập vai hành động sắp ra mắt được phát triển bởi FromSoftware và được xuất bản bởi Bandai Namco Entertainment.'
    ...
    <div className='home__slider-new-game'>
      <div className="home__slider-new-game-container">
            ...
            modules={[Thumbs, Autoplay]}>
              {
                gameItems.map((item, index) => (
                  <SwiperSlide key={index} >
                    ...
                    <div className="home__slider-new-game__img--large-container"
                      style={{backgroundImage: `url(${item.img})`}}>
                        <div className={`home__slider-new-game__img--large-content ${isActive ? 'active' : ''}`}>
                          <div className="home__slider-new-game__img--large-wrapper">
                            ...
                            <p className={`home__slider-new-game__img--large-content-price  ${item.discount != 0 && 'home__slider-new-game__img--large-content-old'}`}>
                              {(item.price * 1).toLocaleString('en-US')}đ
                            </p>
                            <p className='home__slider-new-game__img--large-content-description'>{item.description}</p>
                            <h1>{item.name}</h1>
                          </div>
                        </div>
                    </div>
        ...
        <div className="home__slider-new-game__wrapper">
          <Swiper className='home__slider-new-game__slider'
                ...
                <SwiperSlide key={index}>
                  {
                    ({isActive }) => {
                      ...
                      // }}
                        className={`home__slider-new-game__img--small ${isActive ? 'active' : ''}`}
                        style={{backgroundImage: `url(${item.img})`}}
                      >
                      </div>)
                    }
